...
# --- Configuration ---
GITHUB_REPO="spineldb/spinel-cli"
BINARY_NAME="spinel-cli"
...
setup_environment() {
  ...
  if [ -n "$TERMUX_VERSION" ] || [ -d "/data/data/com.termux/files/usr" ]; then
    ...
    msg "Please install SpinelDB CLI by compiling from source."
    echo ""
    echo "1. Install dependencies:"
    echo "   pkg install rust git"
    echo ""
    echo "2. Clone the repository and compile:"
    echo "   git clone https://github.com/spineldb/spinel-cli.git"
    echo "   cd spinel-cli"
    echo "   cargo build --release"
    echo ""
    echo "3. The binary will be located at: ./target/release/spinel-cli"
...
main() {
  ...
  EXPECTED_ASSET_FILENAME="spinel-cli-${TAG}-${OS_ARCH_COMBO}.tar.gz"
